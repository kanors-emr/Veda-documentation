...
!!! warning "Warning"
    ...
    **Parallel processing requires unique declarations**: Because
    `~FI_process` and `~FI_commodity` tags are processed in parallel, duplicate
    ...
    attributes. See the `~FI_COMM` and `~FI_PROCESS` sections in VedaTags
...
- **All files selected for synchronization are scanned for Veda tags, in
  ...
  PostgreSQL. Tags are processed in the following order:**
    ...
    - **TOPDINS and TOPINS**
        ...
        - Across all files - BY_Trans, SubRES_Trans and scenario
          files.
