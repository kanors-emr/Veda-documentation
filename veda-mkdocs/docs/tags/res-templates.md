...
The **Commodity Definition Table (~FI_COMM)** is used to declare the
...
The **~FI_COMM** table is supported in B-Y templates, SubRES files,
...
Valid column headers for the **~FI_COMM** table are described in Table
...
!!! warning "Warning"
    ...
    **Avoid declaring the same Commodity or Process with conflicting
    ...
    commodity (e.g., `ELC`) or process may have different attributes in each
    run if it is declared with conflicting attributes—such as different Time
    Slice Levels (**TSLVL**) in the `~FI_Comm` or `~FI_Process` tags—across multiple scenarios. This makes debugging and interpreting the output of GAMS runs
    difficult.
...
The **~FI_COMM** table is used to declare commodities with their
associated attributes and properties. Each commodity is declared once
with its characteristics.

#### Valid Column Headers

The valid column headers for a **~FI_COMM** table are listed below
...
The **Process Definition Table (~FI_PROCESS)** is used to declare the
...
!!! note "Note"
    ...
    The **~FI_PROCESS** table provides a flexible layout: the column order
...
!!! warning "Warning"
    ...
    **Each process must be declared exactly once** with consistent
    attributes across all templates. Like FI_COMM tags, FI_PROCESS tags
    ...
    processing warning in the FI_COMM section above for details.
...
By defining processes in the **~FI_PROCESS** table, users create a
...
The **Flexible Import Table (~FI_T)** is a versatile table used
